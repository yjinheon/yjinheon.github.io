...
- 의존성 주입시 구현 클래스가 아닌 인터페이스에 의존하는 것은 SOLID OCP와 DIP에 기반한 전략 패턴이다.
	- OCP
		- 컨트롤러 입장에서는 서비스 클래스의 로직에는 관심이 없고 인터페이스의 메소드를 이용해 로직을 구성하기 때문에 내부로직의 변경에 영향받지 않는다(변화에 닫혀있다.)
		- 서비스 입장에서는 인터페이스만 준수하면 로직 변경이 자유롭다.(확장에는 열려있다.)
	- DIP
	    - 인터페이스는 컴파일 시점에 어떤 클래스를 담을 지 결정하지 않는다.
        * 구현 클래스는 컴파일시점에 결정된다.-> 서비스와 컨트롤러의 결합도가 느슨해진다.
